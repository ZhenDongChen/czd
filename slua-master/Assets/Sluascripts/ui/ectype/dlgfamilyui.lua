...
local function OnChange(params)
    ...
    local ally =  params.ally
	local enemy = params.enemy
    OnTeam(true,ally)
    OnTeam(false,enemy)
    -- if params.IsTeamRed then 
    --     fields.UILabel_My.color = Color(250/255, 7354/255, 38/255, 1)
    -- else
    --     fields.UILabel_My.color = Color(154/255, 254/255, 25/255, 1)
    -- end
end
    ...
    for i=1,3 do
        ...
        EventHelper.SetClick(fields[compName],function()
            ...
            local param = {
                ...
                lengthCallback = {{length=1200,callback=AutoFight}},
...
local function OnScore(param)
	local myScore 
	local yourScore
    local myScore  = param.myfamilyscore
    local yourScore = param.opponentscore
    fields.UILabel_MyScore.text = myScore
    fields.UILabel_YourScore.text = yourScore
end
